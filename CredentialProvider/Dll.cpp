...
extern HRESULT CUnespProvider_CreateInstance(__in REFIID riid, __deref_out void** ppv);
EXTERN_C GUID CLSID_CUnespProvider;
...
public:
    ...
    IFACEMETHODIMP CreateInstance(__in IUnknown* pUnkOuter, __in REFIID riid, __deref_out void **ppv)
    {
        ...
        if (!pUnkOuter)
        {
            hr = CUnespProvider_CreateInstance(riid, ppv);
        }
        else
        {
            *ppv = NULL;
...
HRESULT CClassFactory_CreateInstance(__in REFCLSID rclsid, __in REFIID riid, __deref_out void **ppv)
{
    *ppv = NULL;
    ...
    if (CLSID_CUnespProvider == rclsid)
    {
